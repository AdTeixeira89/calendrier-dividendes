...
/* Tableau Compte × Zone géographique (US / Europe / Autre) + total général.
   ...
   pour donner une vue d'ensemble complète en un coup d'œil. */
function renderRegionAccountTable(year){
  ...
  const bodyRows = rows.map(r=>`
    <tr>
      ...
      <td class="mono region-total-col">${pillsFromTotals(r.accountTotal)}</td>
    </tr>`).join('');
    ...
    <table class="region-totals-table">
      ...
      <tbody>
        ...
        <tr class="region-grand-total">
          ...
          <td class="mono region-total-col">${pillsFromTotals(grandTotal)}</td>
        </tr>
      </tbody>
    </table>
  </div>`;
...
function renderTotalsBlock(year, currencies){
  ...
  const overallPills = currencies.map(cur=>
    `<span class="chart-total-pill">${escapeHtml(cur)} <strong>${fmtAmount2(overall[cur]||0, cur)}</strong></span>`
  ).join('');

  return `<div class="chart-totals-wrap">
    <div class="chart-totals">Total ${year} : ${overallPills}</div>
...
function renderStats(){
  ...
  // Le modèle "mois seul" n'ayant plus d'historique de dates réelles,
  // on affiche simplement une fenêtre de 2 ans avant à 2 ans après l'année en cours.
