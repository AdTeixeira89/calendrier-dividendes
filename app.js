...
function renderTotalsBlock(year, currencies){
  ...
  if(view.accountFilter === 'all' && state.accounts.length > 1){
    const rows = state.accounts.map(acc=>{
      const accHoldings = state.holdings.filter(h=>h.account===acc.id);
      if(!accHoldings.length) return '';
      const totals = totalsByCurrencyForHoldings(accHoldings, rangeStart, rangeEnd);
      const pills = currencies
        .filter(cur=> totals[cur])
        .map(cur=> `${fmtAmount2(totals[cur], cur)}`)
        .join(' + ');
      if(!pills) return '';
      return `<div class="chart-account-row"><span>${escapeHtml(acc.name)}</span><span class="mono">${pills}</span></div>`;
    }).join('');
    if(rows) perAccountHtml = `<div class="chart-account-breakdown">${rows}</div>`;
  }
